...
%nFrames=100; %solo per exp 8!!!
% ##################################################################################################################################################
%                                                                                                                                              LOAD:
% ##################################################################################################################################################
% Carichiamo input--------------------------
name_mtx1 = [roots,names{cexp,2},'/',output,'/VortPot_time_',num2str(Nraggi),'_',num2str(Ncerchi),'_',num2str(nFrames)];
[Pvpolrm,b]=loadmtx(name_mtx1);
name_mtx2 = [roots,names{cexp,2},'/',output,'/Vz_',num2str(Nraggi),'_',num2str(Ncerchi),'_',num2str(nFrames)];
[Vz,b]=loadmtx(name_mtx2);
%   b(2)=2
...
%                                                                                                                                   NUMERO DEI DATI:
% ##################################################################################################################################################
Ndata_raggio_t = zeros(Nraggi,b(2));
Ndata_cerchi_t = zeros(Ncerchi,b(2));
for it=1:b(2)
    pvPolr=reshape(Pvpolrm(:,it),Nraggi,Ncerchi);
...
PtDaTogliere = [1, 178, 179,180];%[42,157,158,159,160];

% ##################################################################################################################################################
%                                                                                                                                   LOOP SUL TEMPO:
% ##################################################################################################################################################
Mono_rpt = zeros(Nraggi,Ncerchi,b(2));
Lm_t = zeros(Nraggi,b(2));
Mono_rpt_zm = zeros(Ncerchi,b(2));
%
VortPot_zm = nan(Ncerchi,1);
VortPot_zmt = zeros(Ncerchi,b(2));
%
Vz_zm = nan(Ncerchi,1);
Vz_zmt = zeros(Ncerchi,b(2));
%
dVortPot_zm_dr = nan(Ncerchi,1);
dVortPot_zmt_dr= zeros(Ncerchi,b(2));

for it=1:b(2)
it
pvPolr=reshape(Pvpolrm(:,it),Nraggi,Ncerchi);
Vz_t=reshape(Vz(:,it),Nraggi,Ncerchi);
% Togliamo i punti con pocchi dati schelto in PtDaTogliere;
pvPolr(:,PtDaTogliere) = nan;
Vz_t(:,PtDaTogliere) = nan;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
a=size(pvPolr);
LL_m = zeros(a(1),1);
Mono_rp = zeros(a);
    ...
    for iR = 1:Ncerchi
        ...
        elseif (~isnan(VortPot_zm(iR)) == 1 & iR == Ncerchi)% derivata all indietro
...
%              less convicing.
% Procedure II: medie temporale e spaziale sui profili di vorticità
%               Facciamo la media zonale e temporale dei profili di
%               vorticità. Dopo monotoniziamo e esce la scala su un solo 
%               profilo mediato. --> Averaging procedure are likely to kill
%               turbulence properties
% Procedure III: media spaziale sulla vorticità e temporale sulla scala 
%               Facciamo la media zonale della vorticità, monotoniziamo
...
% camera.
criterio = 50;
disp('Attenzione la scala di Thorpe deve essere al massimo la metà di:')
...
ylabel('Radius (m)')
title(['Zonal and temporal average of the PV '])%--> L_M = ',num2str(LII_m),' (m).'])
...
xlabel('Potential Vorticity (m^{-1}.s^{-1})')
ylabel('Radius (m)')
title(['Instantaneous profiles of the PV zonal mean'])% & <L_M>_T = ',num2str(LIIIm_mt),' (m).'])
legend('Potential vorticity','Monotonized vorticity')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%        THORPE SCALE
